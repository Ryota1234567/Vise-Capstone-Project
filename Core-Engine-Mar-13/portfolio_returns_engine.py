...
if _OPTIMIZER_AVAILABLE:
    ...
    if enable_optimizer:
        ...
        st.sidebar.markdown("**TLH Proxy Tickers** *(one per line: ORIGINAL,PROXY)*")
        _proxy_raw = st.sidebar.text_area(
            "Proxy pairs", value="", height=80, key="opt_proxies",
            help="Map each ticker to a substitute for the 30-day wash-sale window. e.g. SPY,VOO",
        )
        ...
        # Parse proxy pairs into DataFrame
        _proxy_rows = []
        for _line in _proxy_raw.splitlines():
            _parts = [p.strip().upper() for p in _line.split(",") if p.strip()]
            if len(_parts) >= 2:
                _proxy_rows.append({"symbol": _parts[0], "lookup_type": "TLH_PROXY",
                                     "lookup_symbol": _parts[1], "order": 1})
        _proxy_df_ui = pd.DataFrame(_proxy_rows) if _proxy_rows else None
    else:
    ...
    if enable_optimizer:
        ...
        if _static_result and _opt_result_full:
            ...
            for _lbl_o, _res_o in [("Static (TLH only)", _static_result), ("Optimized (Rebal+TLH)", _opt_result_full)]:
                ...
                _ta1 = _res_o.get("tax_alpha_1_final", np.nan)
                _ta2 = _res_o.get("tax_alpha_2_final", np.nan)
                _ta1_pct = (_ta1 / float(initial_capital)) if (isinstance(_ta1, (int, float)) and float(initial_capital) > 0) else np.nan
                ...
                _tlh_rows_opt.append({
                    ...
                    "Tax Alpha 1 ($)": f"${_ta1:+,.0f}" if np.isfinite(_ta1) else "—",
                    "Tax Alpha 1 (% cap)": f"{_ta1_pct:+.2%}" if np.isfinite(_ta1_pct) else "—",
...
if opt:
    ...
    _alpha1_s = opt_result_d.get("tax_alpha_1_series")
    _alpha2_s = opt_result_d.get("tax_alpha_2_series")
    if _alpha1_s is not None or _alpha2_s is not None:
        st.markdown("#### Tax Alpha Over Time (Optimized Run)")
        _alpha_chart_df = pd.DataFrame(index=opt_result_d["nav_series"].index)
        if _alpha1_s is not None:
            _alpha_chart_df["Alpha 1: TLH vs no-realization benefit ($)"] = _alpha1_s.values
        ...
        st.caption(
            "Alpha 1: value added by realizing losses (same trades, shadow engine ignores TLH tax benefit). "
            "Alpha 2: value of TLH over a buy-and-hold-rebalance baseline with no harvesting."
        )
