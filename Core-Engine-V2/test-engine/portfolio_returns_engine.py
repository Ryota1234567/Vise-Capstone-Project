...
from portfolio_shared import (
    ensure_data, load_data, prepare_price_data,
    compute_strategy_metrics, DATA_PATH,
)
...
# load_data, compute_strategy_metrics, prepare_price_data, ensure_data, DATA_PATH
# are all imported from portfolio_shared at the top of this file.
...
if _OPTIMIZER_AVAILABLE:
    ...
    if enable_optimizer:
        ...
        opt_fixed_cost = st.sidebar.number_input(
            "Fixed Cost per Trade ($)", min_value=0.0, max_value=100.0,
            value=0.0, step=0.50, format="%.2f", key="opt_fixed_cost",
            help="Flat dollar commission charged on each buy or sell order",
        )
        opt_variable_bps = st.sidebar.number_input(
            "Variable Cost (bps)", min_value=0.0, max_value=100.0,
            value=0.0, step=0.5, format="%.1f", key="opt_variable_bps",
            help="Variable transaction cost as basis points of trade value (e.g. 5 = 0.05%)",
        )
        opt_slippage_bps = st.sidebar.number_input(
            "Slippage (bps)", min_value=0.0, max_value=100.0,
            value=0.0, step=0.5, format="%.1f", key="opt_slippage_bps",
            help="Market-impact slippage as basis points of trade value (e.g. 5 = 0.05%)",
        )
        _opt_trading_costs = TradingCosts(
            fixed_cost_per_trade=opt_fixed_cost,
            variable_cost_bps=opt_variable_bps,
            slippage_bps=opt_slippage_bps,
        )
    else:
...
if enable_optimizer:
    ...
    with st.spinner("Running MSBA v1 Static simulation..."):
        try:
            static_result = run_optimizer_simulation(
                ...
                bid_field="PRICEBID", ask_field="PRICEASK",
                trading_costs=_opt_trading_costs,
                static=True,
            )
    ...
    if static_result and opt_result:
        ...
        kc5.metric("Trading Costs (Opt)", f"${opt_result['costs_paid_total']:,.2f}", delta=f"Static: ${static_result['costs_paid_total']:,.2f}")
